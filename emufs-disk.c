	...
	else
	{
		...
		if(superblock->fs_number==EMUFS_ENCRYPTED){
			...
			/*
				Decrypt Here
			*/
			decrypt(key,tempBuf,sizeof(int));
			memcpy(superblock, tempBuf, sizeof(struct superblock_t));
		}
...
void read_superblock(int mount_point, struct superblock_t *superblock){
	...
	*/
	...
	if(mounts[mount_point].fs_number==EMUFS_ENCRYPTED){
		decrypt(mounts[mount_point].key,tempBuf,sizeof(int));
	}
	memcpy(superblock, tempBuf, sizeof(struct superblock_t));
}
...
void write_superblock(int mount_point, struct superblock_t *superblock){
	...
	*/
	...
	memcpy(tempBuf, superblock, sizeof(struct superblock_t));
	if(superblock->fs_number==EMUFS_ENCRYPTED){
		encrypt(mounts[mount_point].key,tempBuf,sizeof(int));
	}
	...
	*/
	...
	for(int i=0;i<32;i++){
		if(superblock.inode_bitmap[i]==0){
			// printf("got a free inode at %d",&i);
...
void read_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	...
	*/
	char tempBuf[BLOCKSIZE];
	if(inodenum<16) readblock(mounts[mount_point].device_fd, 1, tempBuf);
	else readblock(mounts[mount_point].device_fd, 2, tempBuf);
	if(mounts[mount_point].fs_number==1) decrypt(mounts[mount_point].key,tempBuf,BLOCKSIZE);
...
void write_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	...
	*/
	...
	if(mounts[mount_point].fs_number==1) decrypt(mounts[mount_point].key,tempBuf,BLOCKSIZE);
	memcpy(tempBuf+(inodenum%16)*sizeof(struct inode_t),inodeptr, sizeof(struct inode_t));
	if(mounts[mount_point].fs_number==1) encrypt(mounts[mount_point].key,tempBuf,BLOCKSIZE);
	if(inodenum<16) writeblock(mounts[mount_point].device_fd, 1, tempBuf);
	else writeblock(mounts[mount_point].device_fd, 2, tempBuf);
	return;
}
